...
export async function submitWorker(ctx: WorkerCommandContext, input: SubmitWorkerInput) {
  try {
    ...
    for (const artifact of input.input.artifacts) {
      ...
      if ((artifact.source.startsWith('code:') && type !== 'code_change') || (artifact.source.startsWith('blob:') && type === 'code_change')) throw new ExecutionError('artifact source/type mismatch');
  ...
  try {
    ...
    if (state.state === 'failed') {
      ...
      if (notes === undefined && spec.artifacts.some((a) => a.source === 'blob:work-notes')) {
        ...
        await completeRun(ctx, { ...input, workerOutput: state.workerOutput, artifacts: spec.artifacts,
