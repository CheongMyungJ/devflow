...
4. `scope` 위반 여부를 확인한다. 단, 실행 중 사람의 지시로 범위가 바뀐 경우는 위반이 아니다. Task 정의에 `non_goals`(범위 밖)가 있으면 산출물이 그것을 넘지 않았는지도 본다 — Step 의 `scope` 가 허용하는 것처럼 보이더라도 넘었으면 지적으로 적는다.
...
- **임시 파일은 자신이 만든 디렉터리 하나에만 쓰고 그것만 지운다.** 그 디렉터리는 Workspace 밖(OS 의 임시 위치)에 만든다. repo 권한은 읽기다 — Workspace 의 추적 파일(git 이 추적하는 파일)과 실제 기록(시스템의 상태 기록)을 고치지 않는다. 자신이 직접 쓰는 것은 그 디렉터리의 파일과 출력 파일뿐이다. Workspace 에서 검사를 실행하는 것은 괜찮고, 실행으로 생기는 git 이 무시하는 생성물도 괜찮다. 고쳐 보거나 되돌려 보는 확인은 그 디렉터리에 복사본을 만들어 거기서 한다.
