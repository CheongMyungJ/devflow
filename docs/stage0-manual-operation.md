...
- **Ledger 와 손으로 쓰는 YAML 은 편집 도구로 고친다** — 문자열 치환 스크립트(예: node 의 `String.replace`)로 고치지 않는다. 치환 문자열의 `$` 가 패턴으로 해석되어 문서가 깨진 채 commit 된 적이 있다(T-0005). heredoc 안의 `\` 는 YAML 이스케이프 오류를 낼 수 있다 — 작은따옴표 YAML 문자열로 쓴다.
