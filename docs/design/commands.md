...
- **Step 의 status 를 바꾸는 command 는 step.yaml 의 `status` 와 `step.status_changed` 를 같은 commit 에 쓴다.** 한 commit 에 전이가 둘이면(승인의 in_review→approved→closed) step.yaml 에는 마지막 `to` 를 쓰고 `step.status_changed` 는 걸음마다 하나씩 쓴다. status 를 바꾸지 않는 command 는 `step.status_changed` 를 쓰지 않는다. Step 이 처음 생길 때(`recordDecision` 의 proposed)는 `step.status_changed` 를 쓰지 않고 step.yaml 과 `step.proposed` 를 같은 commit 에 쓴다 — 기존 기록에도 proposed 로의 `step.status_changed` 가 없고, `from` 이 없는 전이를 새로 만들지 않는다(T-0006 step-004, D-004). 그래서 어느 command 뒤에도 **"step.yaml 의 status = 그 Step 의 status 를 정한 마지막 이벤트"** — `step.status_changed` 가 있으면 그 마지막 것의 `to`, 없으면 `step.proposed` 의 proposed — 다(T-0006 AC3 의 "마지막 step.status_changed 의 to" 를 이렇게 읽는다. `tests/step-round-flow.test.ts` 가 명령마다 이것을 검사한다).
...
- artifacts: Step 의 `outputs` 에 선언된 이름마다 하나. `<source>` 는 `code:<base-sha>..<head-sha>`(code_change — repo·branch 는 Task 의 target 에서), `repo:<base-sha>..<head-sha>:<path>[,<path>…]`(대상 repo 안의 문서, `stored_in: repo`), `blob:work-notes`(이 commit 의 작업 노트 blob, `stored_in: store` — 지금 받는 blob 출처는 이것 하나다).
- 도구가 채우는 것: 버전(`nextArtifactVersion`), ref, task_id, step_id, run_id, author worker, stored_in, content_key(= `blobRef`), 작업 노트가 있으면 모든 산출물의 work_notes_key, created_at, Run 의 completed·ended_at·output_attempts·packet_gaps, status(running·revising → checking).
...
**`addFeedback(ctx, { taskId, stepId?, kind, channel, target?: { artifactRef?, location?, runId?, decisionId? }, text })`** — **구현됨**(`src/commands/review.ts`). 입구 `add-feedback <data-dir> <task-id> --kind <kind> --channel <channel> [--step <step-id>] [--artifact-ref <ref>] [--location <text>] [--run R-NNN] [--decision D-NNN] (--text | --text-file) --actor human:<id>`. kind 는 question·direction·requirement·answer 만이다(approval 은 `approveStep`, revision_request 는 `requestRevision` 이 쓴다 — 한 기록은 한 command). target 의 artifact 참조는 문법과 존재(stepId 가 있으면 그 Step 의 것)를 본다 — 가장 새 버전일 필요는 없다(질문은 옛 버전에 대해서도 한다). target 의 Run·Decision 은 그 Task 에 있어야 한다. `feedback.added` 의 data 는 kind, channel. status 는 그대로.
...
command 별로 더 거부하는 것(구현된 것은 테스트가 조건마다 있다 — `tests/commands/runs-gates.test.ts`, `tests/commands/plan-review.test.ts`, 입구는 `tests/record-gate.test.ts`, `tests/run-entries.test.ts`, `tests/plan-entries.test.ts`):
...
- `recordDecision`: Run 이 없거나 planner 가 아니거나 submitted 가 아니다. 출력이 객체가 아니다. 도구가 채운 뒤의 Decision 이나 제안된 Step 이 스키마에 맞지 않다. next_step(step 이 있는 것)인데 닫히지 않은(closed·cancelled 가 아닌) Step 이 있다 — 0단계는 Step 을 하나씩 돈다.
...
- `addFeedback`: actor 가 사람이 아니다. text 가 비었다. kind 가 question·direction·requirement·answer 밖이다(approval·revision_request 는 다른 command). 없는 Step. target 의 artifact 참조가 문법·Task(와 Step)·존재에 맞지 않다, target 의 Run·Decision 이 그 Task 에 없다, target 에 모르는 필드가 있다.
- `completeTask`(아직 없다): decisionId 의 Decision 이 action done 이 아니다. 닫히지 않은 Step 이 있다. merge sha 가 40자(또는 64자) 16진 소문자가 아니다.
