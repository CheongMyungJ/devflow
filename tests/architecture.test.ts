...
describe('구조 제약 — scripts/', () => {
  ...
  const ENTRIES = ['append-events', 'submit-run', 'complete-run', 'fail-run', 'record-gate', 'propose-step', 'define-step', 'request-revision', 'approve-step', 'add-feedback'].map((name) => `scripts/${name}.mjs`);
